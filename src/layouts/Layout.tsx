  ...
  return (
    <div className="grid md:grid-cols-[16rem_1fr] grid-cols-1 min-h-screen w-screen bg-gray-800">
      ...
      <nav className="fixed bottom-0 left-0 right-0 text-white bg-gray-900 flex justify-around mb-10 text-xl md:hidden">
        <Link to="/" className={`flex flex-col items-center w-full h-full  hover:text-green-500 hover:bg-gray-700 p-10 ${isActive('/') ? 'text-green-500 bg-gray-700' : 'text-white'}`}>
          <IoMdHome />
        </Link>
        <Link to="/planificaciones" className={`flex flex-col items-center w-full h-full hover:text-green-500 hover:bg-gray-700 p-10 ${isActive('/planificaciones') ? 'text-green-500 bg-gray-700' : 'text-white'}`}>
          <FaDumbbell />
        </Link>
        <Link to="/turnos" className={`flex flex-col items-center w-full h-full hover:text-green-500 hover:bg-gray-700 p-10 ${isActive('/turnos') ? 'text-green-500 bg-gray-700' : 'text-white'}`}>
          <FaCalendarAlt />
        </Link>
        <Link to="/videos" className={`flex flex-col items-center w-full h-full hover:text-green-500 hover:bg-gray-700 p-10 ${isActive('/videos') ? 'text-green-500 bg-gray-700' : 'text-white'}`}>
          <MdSlowMotionVideo />
        </Link>
        {isAdmin && (
          <Link to="/admin" className={`flex flex-col items-center w-full h-full hover:text-green-500 hover:bg-gray-700 p-10 ${isActive('/admin') ? 'text-green-500 bg-gray-700' : 'text-white'}`}>
            <FaUserShield />
          </Link>
        )}
